...
```html
<form action="/login(백엔드쪽 엔드포인트이름)" method="get">
  <input>
  <button>로그인</button>
</form>
```
-> 버튼 클릭시 브라우저가 action="/login" 주소로 데이터를 보낸다.
-> 3000 -> 8000
-> 페이지가 새로고침되거나 이동됨
...
예시상황
- form태그안에 버튼이 있을 때 -> submit발생함 -> 리액트 리렌더링발생으로 문제가 됨
...
해결방법
첫번째
- form의 기본 submit동작을 막는다 -> e.preventDefault()
두번째
- submit이 아니라 button으로 바꾼다.
```html
