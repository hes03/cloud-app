...
- Evaluation(코드실행) - Caching(결과캐싱)
-> 내장모듈 -> 파일경로 탐색 -> 패키지 모듈 탐색(node_modules-express)
...
-> Evalution: 래핑된 코드를 실행하여 모듈 내 코드를 해석하는 과정
(모듈의 내보내기는 이 과정에서 결정됨)
-> Cacing: 로딩된 모듈을 캐싱하는 과정
(같은 모듈을 요청하면 메모리서 결과를 즉시 반환-속도)
*/ 
